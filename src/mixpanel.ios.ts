  ...
  static init(token) {
    if (token) {
      if (Mixpanel) {
        ...
        console.log(
          "MIXPANEL PLUGIN: We can not find the cocoapod, make sure is added during building"
        );
      }
    } else {
  ...
  static track(eventName, props?) {
    if (mixpanel) {
      if (eventName) {
        ...
        if (props) {
          const iosPropsMutable = NSMutableDictionary.alloc().init();
          for (const key in props) {
            iosPropsMutable.setObjectForKey(props[key] + "", key + "");
          }
          const iosProps = NSDictionary.dictionaryWithDictionary(
            iosPropsMutable
          );
  ...
  static identify(id, extraAtributes?) {
    if (mixpanel) {
      if (id) {
        ...
        if (extraAtributes) {
          const iosPropsMutable = NSMutableDictionary.alloc().init();
          for (const key in extraAtributes) {
  ...
  static registerSuperProperties(props) {
    if (mixpanel) {
      if (props) {
        ...
        const iosProps = NSDictionary.dictionaryWithDictionary(iosPropsMutable);
